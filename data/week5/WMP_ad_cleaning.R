##### ad cleaning #####
# 2006
library(haven)
wmp_federal_2006_v1_0_1 <- read_dta("WMP/House ads/wmp-federal-2006-v1.0-1.dta")
# install.packages('usdata')
library(usdata)
library(tidyverse)

house_2006 <- wmp_federal_2006_v1_0_1 %>%
  filter(race == 'HOUSE') %>%
  sjlabelled::as_label(party, ad_tone, per_ply) %>%
  select(creative, market, dma, airdate, airtime, categorystate, 
         race, district, party, ad_tone, per_ply, station) %>%
  mutate(cycle = substr(airdate,0,4)) %>%
  rename(state = categorystate) %>%
  mutate(state = abbr2state(state) ) %>%
  mutate(st_fips = case_when(state == 'Alabama' ~ '01',
                             state == 'Alaska' ~ '02',
                             state == 'Arizona' ~ '04',
                             state == 'Arkansas' ~ '05',
                             state =='California' ~ '06',
                             state =='Colorado' ~ '08',
                             state =='Connecticut' ~ '09',
                             state =='Delaware' ~ '10',
                             state == 'Florida' ~ '12',
                             state == 'Georgia' ~ '13',
                             state == 'Hawaii' ~ '15',
                             state =='Idaho' ~ '16',
                             state =='Illinois' ~ '17',
                             state =='Indiana' ~ '18',
                             state =='Iowa' ~ '19',
                             state =='Kansas' ~ '20',
                             state =='Kentucky' ~ '21',
                             state =='Louisiana' ~ '22',
                             state =='Maine' ~ '23',
                             state =='Maryland' ~ '24',
                             state =='Massachusetts' ~ '25',
                             state =='Michigan' ~ '26',
                             state =='Minnesota' ~ '27',
                             state =='Mississippi' ~ '28',
                             state =='Missouri' ~ '29',
                             state =='Montana' ~ '30',
                             state =='Nebraska' ~ '31',
                             state =='Nevada' ~ '32',
                             state =='New Hampshire' ~ '33',
                             state =='New Jersey' ~ '34',
                             state =='New Mexico' ~ '35',
                             state =='New York' ~ '36',
                             state =='North Carolina' ~ '37',
                             state =='North Dakota' ~ '38',
                             state =='Ohio' ~ '39',
                             state =='Oklahoma' ~ '40',
                             state =='Oregon' ~ '41',
                             state =='Pennsylvania' ~ '42',
                             state =='Rhode Island' ~ '44',
                             state =='South Carolina' ~ '45',
                             state =='South Dakota' ~ '46',
                             state =='Tennessee' ~ '47',
                             state =='Texas' ~ '48',
                             state =='Utah' ~ '49',
                             state =='Vermont' ~ '50',
                             state =='Virginia' ~ '51',
                             state =='Washington' ~ '53',
                             state =='West Virginia' ~ '54',
                             state =='Wisconsin' ~ '55',
                             state =='Wyoming' ~ '56')) 
head(house_2006)

# add st_cd_fips
nums <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
house_2006$district <- as.numeric(house_2006$district)
table(house_2006$district)
house_2006$cd_fips <- ifelse(house_2006$district %in% nums, 
                             paste0("0",house_2006$district), 
                             house_2006$district)

house_2006$st_cd_fips <- paste(house_2006$st_fips,
                               house_2006$cd_fips,
                               sep = "")
head(house_2006)

# 2010
wmp_federal_2010_v1_3_1 <- read_dta("WMP/House ads/wmp-federal-2010-v1.3-1.dta")
house_2010 <- wmp_federal_2010_v1_3_1 %>%
  filter(race == 'US House') %>%
  sjlabelled::as_label(party, ad_tone, per_ply) %>%
  select(creative, market, dma, airdate, airtime, state, 
         race, district, party, ad_tone, per_ply, station) %>%
  mutate(cycle = substr(airdate,0,4)) %>%
  mutate(state = abbr2state(state) ) %>%
  mutate(st_fips = case_when(state == 'Alabama' ~ '01',
                             state == 'Alaska' ~ '02',
                             state == 'Arizona' ~ '04',
                             state == 'Arkansas' ~ '05',
                             state =='California' ~ '06',
                             state =='Colorado' ~ '08',
                             state =='Connecticut' ~ '09',
                             state =='Delaware' ~ '10',
                             state == 'Florida' ~ '12',
                             state == 'Georgia' ~ '13',
                             state == 'Hawaii' ~ '15',
                             state =='Idaho' ~ '16',
                             state =='Illinois' ~ '17',
                             state =='Indiana' ~ '18',
                             state =='Iowa' ~ '19',
                             state =='Kansas' ~ '20',
                             state =='Kentucky' ~ '21',
                             state =='Louisiana' ~ '22',
                             state =='Maine' ~ '23',
                             state =='Maryland' ~ '24',
                             state =='Massachusetts' ~ '25',
                             state =='Michigan' ~ '26',
                             state =='Minnesota' ~ '27',
                             state =='Mississippi' ~ '28',
                             state =='Missouri' ~ '29',
                             state =='Montana' ~ '30',
                             state =='Nebraska' ~ '31',
                             state =='Nevada' ~ '32',
                             state =='New Hampshire' ~ '33',
                             state =='New Jersey' ~ '34',
                             state =='New Mexico' ~ '35',
                             state =='New York' ~ '36',
                             state =='North Carolina' ~ '37',
                             state =='North Dakota' ~ '38',
                             state =='Ohio' ~ '39',
                             state =='Oklahoma' ~ '40',
                             state =='Oregon' ~ '41',
                             state =='Pennsylvania' ~ '42',
                             state =='Rhode Island' ~ '44',
                             state =='South Carolina' ~ '45',
                             state =='South Dakota' ~ '46',
                             state =='Tennessee' ~ '47',
                             state =='Texas' ~ '48',
                             state =='Utah' ~ '49',
                             state =='Vermont' ~ '50',
                             state =='Virginia' ~ '51',
                             state =='Washington' ~ '53',
                             state =='West Virginia' ~ '54',
                             state =='Wisconsin' ~ '55',
                             state =='Wyoming' ~ '56')) 
head(house_2010)

# add st_cd_fips
nums <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
house_2010$cd_fips <- ifelse(house_2010$district %in% nums, 
                             paste0("0",house_2010$district), 
                             house_2010$district)

house_2010$st_cd_fips <- paste(house_2010$st_fips,
                               house_2010$cd_fips,
                               sep = "")
head(house_2010)

# 2012
wmp_house_2012_v1_1 <- read_dta("WMP/House ads/wmp-house-2012-v1.1.dta")
house_2012 <- wmp_house_2012_v1_1 %>%
  filter(race == 'US HOUSE') %>%
  sjlabelled::as_label(party, ad_tone, per_ply) %>%
  select(creative, market, dmaid, airdate, airtime, categorystate,
         race, district, party, ad_tone, per_ply, station) %>% # issue not included bc diff dimensions
  mutate(cycle = substr(airdate,0,4)) %>%
  rename(state = categorystate, dma = dmaid) %>%
  mutate(state = abbr2state(state) ) %>%
  mutate(st_fips = case_when(state == 'Alabama' ~ '01',
                             state == 'Alaska' ~ '02',
                             state == 'Arizona' ~ '04',
                             state == 'Arkansas' ~ '05',
                             state =='California' ~ '06',
                             state =='Colorado' ~ '08',
                             state =='Connecticut' ~ '09',
                             state =='Delaware' ~ '10',
                             state == 'Florida' ~ '12',
                             state == 'Georgia' ~ '13',
                             state == 'Hawaii' ~ '15',
                             state =='Idaho' ~ '16',
                             state =='Illinois' ~ '17',
                             state =='Indiana' ~ '18',
                             state =='Iowa' ~ '19',
                             state =='Kansas' ~ '20',
                             state =='Kentucky' ~ '21',
                             state =='Louisiana' ~ '22',
                             state =='Maine' ~ '23',
                             state =='Maryland' ~ '24',
                             state =='Massachusetts' ~ '25',
                             state =='Michigan' ~ '26',
                             state =='Minnesota' ~ '27',
                             state =='Mississippi' ~ '28',
                             state =='Missouri' ~ '29',
                             state =='Montana' ~ '30',
                             state =='Nebraska' ~ '31',
                             state =='Nevada' ~ '32',
                             state =='New Hampshire' ~ '33',
                             state =='New Jersey' ~ '34',
                             state =='New Mexico' ~ '35',
                             state =='New York' ~ '36',
                             state =='North Carolina' ~ '37',
                             state =='North Dakota' ~ '38',
                             state =='Ohio' ~ '39',
                             state =='Oklahoma' ~ '40',
                             state =='Oregon' ~ '41',
                             state =='Pennsylvania' ~ '42',
                             state =='Rhode Island' ~ '44',
                             state =='South Carolina' ~ '45',
                             state =='South Dakota' ~ '46',
                             state =='Tennessee' ~ '47',
                             state =='Texas' ~ '48',
                             state =='Utah' ~ '49',
                             state =='Vermont' ~ '50',
                             state =='Virginia' ~ '51',
                             state =='Washington' ~ '53',
                             state =='West Virginia' ~ '54',
                             state =='Wisconsin' ~ '55',
                             state =='Wyoming' ~ '56')) 
head(house_2012)

# add st_cd_fips
nums <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
house_2012$cd_fips <- ifelse(house_2012$district %in% nums, 
                             paste0("0",house_2012$district), 
                             house_2012$district)

house_2012$st_cd_fips <- paste(house_2012$st_fips,
                               house_2012$cd_fips,
                               sep = "")
head(house_2012)

# 2014
WMP_house_gen_2014_ads_cleaned <- readRDS("~/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/WMP/House ads/WMP_house_gen_2014_ads_cleaned.rds")
house_2014 <- WMP_house_gen_2014_ads_cleaned %>%
  filter(race == 'US HOUSE') %>%
  sjlabelled::as_label(party, ad_tone, per_ply) %>%
  select(creative, market, dma, airdate, airtime, StateAbb,
         race, district, affiliation, ad_tone, per_ply, station) %>% # issue not included bc diff dimensions
  mutate(cycle = substr(airdate,0,4)) %>%
  rename(party = affiliation, state = StateAbb) %>%
  mutate(state = abbr2state(state) ) %>%
  mutate(st_fips = case_when(state == 'Alabama' ~ '01',
                             state == 'Alaska' ~ '02',
                             state == 'Arizona' ~ '04',
                             state == 'Arkansas' ~ '05',
                             state =='California' ~ '06',
                             state =='Colorado' ~ '08',
                             state =='Connecticut' ~ '09',
                             state =='Delaware' ~ '10',
                             state == 'Florida' ~ '12',
                             state == 'Georgia' ~ '13',
                             state == 'Hawaii' ~ '15',
                             state =='Idaho' ~ '16',
                             state =='Illinois' ~ '17',
                             state =='Indiana' ~ '18',
                             state =='Iowa' ~ '19',
                             state =='Kansas' ~ '20',
                             state =='Kentucky' ~ '21',
                             state =='Louisiana' ~ '22',
                             state =='Maine' ~ '23',
                             state =='Maryland' ~ '24',
                             state =='Massachusetts' ~ '25',
                             state =='Michigan' ~ '26',
                             state =='Minnesota' ~ '27',
                             state =='Mississippi' ~ '28',
                             state =='Missouri' ~ '29',
                             state =='Montana' ~ '30',
                             state =='Nebraska' ~ '31',
                             state =='Nevada' ~ '32',
                             state =='New Hampshire' ~ '33',
                             state =='New Jersey' ~ '34',
                             state =='New Mexico' ~ '35',
                             state =='New York' ~ '36',
                             state =='North Carolina' ~ '37',
                             state =='North Dakota' ~ '38',
                             state =='Ohio' ~ '39',
                             state =='Oklahoma' ~ '40',
                             state =='Oregon' ~ '41',
                             state =='Pennsylvania' ~ '42',
                             state =='Rhode Island' ~ '44',
                             state =='South Carolina' ~ '45',
                             state =='South Dakota' ~ '46',
                             state =='Tennessee' ~ '47',
                             state =='Texas' ~ '48',
                             state =='Utah' ~ '49',
                             state =='Vermont' ~ '50',
                             state =='Virginia' ~ '51',
                             state =='Washington' ~ '53',
                             state =='West Virginia' ~ '54',
                             state =='Wisconsin' ~ '55',
                             state =='Wyoming' ~ '56')) 
head(house_2014)

# add st_cd_fips
nums <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
house_2014$cd_fips <- ifelse(house_2014$district %in% nums, 
                             paste0("0",house_2014$district), 
                             house_2014$district)

house_2014$st_cd_fips <- paste(house_2014$st_fips,
                               house_2014$cd_fips,
                               sep = "")
head(house_2014)

# label ad purpose variable
house_2014 <- house_2014 %>%
  mutate(per_ply = case_when(per_ply == 0 ~ "Neither",
                             per_ply == 1 ~ "Personal Characteristics",
                             per_ply == 2 ~ "Policy Matters",
                             per_ply == 3 ~ "Both Personal Characteristics and Policy Matters"),
         ad_tone = case_when(ad_tone == 1 ~ "Contrast",
                             ad_tone == 2 ~ "Promote", 
                             ad_tone == 3 ~ "Attack"))
head(house_2014)

# 2016
WMP_house_gen_2016_ads_cleaned <- readRDS("~/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/WMP/House ads/WMP_house_gen_2016_ads_cleaned.rds")
house_2016 <- WMP_house_gen_2016_ads_cleaned %>%
  filter(race == 'US HOUSE') %>%
  sjlabelled::as_label(party, ad_tone, per_ply) %>%
  select(creative, market, dma, airdate, airtime, categorystate,
         race, district, Candidate_Party, ad_tone, per_ply, station) %>% # issue not included bc diff dimensions
  mutate(cycle = substr(airdate,0,4)) %>%
  rename(party = Candidate_Party, state = categorystate) %>%
  mutate(state = abbr2state(state) ) %>%
  mutate(st_fips = case_when(state == 'Alabama' ~ '01',
                             state == 'Alaska' ~ '02',
                             state == 'Arizona' ~ '04',
                             state == 'Arkansas' ~ '05',
                             state =='California' ~ '06',
                             state =='Colorado' ~ '08',
                             state =='Connecticut' ~ '09',
                             state =='Delaware' ~ '10',
                             state == 'Florida' ~ '12',
                             state == 'Georgia' ~ '13',
                             state == 'Hawaii' ~ '15',
                             state =='Idaho' ~ '16',
                             state =='Illinois' ~ '17',
                             state =='Indiana' ~ '18',
                             state =='Iowa' ~ '19',
                             state =='Kansas' ~ '20',
                             state =='Kentucky' ~ '21',
                             state =='Louisiana' ~ '22',
                             state =='Maine' ~ '23',
                             state =='Maryland' ~ '24',
                             state =='Massachusetts' ~ '25',
                             state =='Michigan' ~ '26',
                             state =='Minnesota' ~ '27',
                             state =='Mississippi' ~ '28',
                             state =='Missouri' ~ '29',
                             state =='Montana' ~ '30',
                             state =='Nebraska' ~ '31',
                             state =='Nevada' ~ '32',
                             state =='New Hampshire' ~ '33',
                             state =='New Jersey' ~ '34',
                             state =='New Mexico' ~ '35',
                             state =='New York' ~ '36',
                             state =='North Carolina' ~ '37',
                             state =='North Dakota' ~ '38',
                             state =='Ohio' ~ '39',
                             state =='Oklahoma' ~ '40',
                             state =='Oregon' ~ '41',
                             state =='Pennsylvania' ~ '42',
                             state =='Rhode Island' ~ '44',
                             state =='South Carolina' ~ '45',
                             state =='South Dakota' ~ '46',
                             state =='Tennessee' ~ '47',
                             state =='Texas' ~ '48',
                             state =='Utah' ~ '49',
                             state =='Vermont' ~ '50',
                             state =='Virginia' ~ '51',
                             state =='Washington' ~ '53',
                             state =='West Virginia' ~ '54',
                             state =='Wisconsin' ~ '55',
                             state =='Wyoming' ~ '56')) 
head(house_2016)

# add st_cd_fips
nums <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
house_2016$cd_fips <- ifelse(house_2016$district %in% nums, 
                             paste0("0",house_2016$district), 
                             house_2016$district)

house_2016$st_cd_fips <- paste(house_2016$st_fips,
                               house_2016$cd_fips,
                               sep = "")
head(house_2016)

# 2018
WMP_house_gen_2018_ads_cleaned <- readRDS("~/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/WMP/House ads/WMP_house_gen_2018_ads_cleaned.rds")
house_2018 <- WMP_house_gen_2018_ads_cleaned %>%
  filter(race == 'US HOUSE') %>%
  sjlabelled::as_label(party, ad_tone, per_ply) %>%
  select(creative, market, dma, airdate, airtime, categorystate,
         race, district, Candidate_Party, ad_tone, per_ply, station) %>% # issue not included bc diff dimensions
  mutate(cycle = substr(airdate,0,4)) %>%
  rename(party = Candidate_Party, state = categorystate) %>%
  mutate(state = abbr2state(state) ) %>%
  mutate(st_fips = case_when(state == 'Alabama' ~ '01',
                             state == 'Alaska' ~ '02',
                             state == 'Arizona' ~ '04',
                             state == 'Arkansas' ~ '05',
                             state =='California' ~ '06',
                             state =='Colorado' ~ '08',
                             state =='Connecticut' ~ '09',
                             state =='Delaware' ~ '10',
                             state == 'Florida' ~ '12',
                             state == 'Georgia' ~ '13',
                             state == 'Hawaii' ~ '15',
                             state =='Idaho' ~ '16',
                             state =='Illinois' ~ '17',
                             state =='Indiana' ~ '18',
                             state =='Iowa' ~ '19',
                             state =='Kansas' ~ '20',
                             state =='Kentucky' ~ '21',
                             state =='Louisiana' ~ '22',
                             state =='Maine' ~ '23',
                             state =='Maryland' ~ '24',
                             state =='Massachusetts' ~ '25',
                             state =='Michigan' ~ '26',
                             state =='Minnesota' ~ '27',
                             state =='Mississippi' ~ '28',
                             state =='Missouri' ~ '29',
                             state =='Montana' ~ '30',
                             state =='Nebraska' ~ '31',
                             state =='Nevada' ~ '32',
                             state =='New Hampshire' ~ '33',
                             state =='New Jersey' ~ '34',
                             state =='New Mexico' ~ '35',
                             state =='New York' ~ '36',
                             state =='North Carolina' ~ '37',
                             state =='North Dakota' ~ '38',
                             state =='Ohio' ~ '39',
                             state =='Oklahoma' ~ '40',
                             state =='Oregon' ~ '41',
                             state =='Pennsylvania' ~ '42',
                             state =='Rhode Island' ~ '44',
                             state =='South Carolina' ~ '45',
                             state =='South Dakota' ~ '46',
                             state =='Tennessee' ~ '47',
                             state =='Texas' ~ '48',
                             state =='Utah' ~ '49',
                             state =='Vermont' ~ '50',
                             state =='Virginia' ~ '51',
                             state =='Washington' ~ '53',
                             state =='West Virginia' ~ '54',
                             state =='Wisconsin' ~ '55',
                             state =='Wyoming' ~ '56')) 
head(house_2018)

# st_cd_fips
nums <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
house_2018$cd_fips <- ifelse(house_2018$district %in% nums, 
                             paste0("0",house_2018$district), 
                             house_2018$district)

house_2018$st_cd_fips <- paste(house_2018$st_fips,
                               house_2018$cd_fips,
                               sep = "")
head(house_2018)

# join now
names(house_2006)
names(house_2010)
names(house_2012)
names(house_2014)
names(house_2016)
names(house_2018)

h <- house_2006 %>%
  rbind(house_2010) %>%
  rbind(house_2012) %>%
  rbind(house_2014) %>%
  rbind(house_2016) %>%
  rbind(house_2018)
dim(h)

# write separate files - if necessary
# write.csv(house_2006, "/Users/kiara/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/house_2006.csv")
# write.csv(house_2010, "/Users/kiara/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/house_2010.csv")
# write.csv(house_2012, "/Users/kiara/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/house_2012.csv")
# write.csv(house_2014, "/Users/kiara/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/house_2014.csv")
# write.csv(house_2016, "/Users/kiara/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/house_2016.csv")
# write.csv(house_2018, "/Users/kiara/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/house_2018.csv")

# check name matching again for ad_tone and party
table(h$party)
h <- h %>%
  mutate(party = case_when(party == 'Democrat' ~ 'Democrat',
                           party == 'DEMOCRAT' ~ 'Democrat',
                           party == 'Democratic' ~ 'Democrat',
                           party == 'REPUBLICAN' ~ 'Republican',
                           party == 'Republican' ~ 'Republican')) %>%
  mutate(ad_tone = tolower(ad_tone)) %>%
  filter(cycle %in% c(2006, 2010, 2012, 2014, 2016, 2018)) %>%
  rename(ad_purpose = per_ply) %>%
  mutate(ad_purpose = tolower(ad_purpose), ad_purpose = case_when(ad_purpose == 'both personal char. and policy matters' ~ 
                                                                    "both",
                                                                  ad_purpose == 'both personal characteristics and policy matters' ~ 
                                                                    "both",
                                                                  ad_purpose == 'personal characteristics' ~
                                                                    "personal",
                                                                  ad_purpose == 'policy matters' ~
                                                                    "policy"
  )) %>%
  group_by(creative) %>%
  mutate(n_stations = length(unique(station))) %>% ungroup()

table(h$party)
table(h$ad_tone)
table(h$ad_purpose)
table(h$cycle)
head(h)
head(h$n_stations)
dim(h)
# export to csv
write.csv(h, "/Users/kiara/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/ads_2006_2018.csv")

#### elections that include issues and cost 2012-2018 ####
# 2012
wmp_house_2012_v1_1 <- read_dta("WMP/House ads/wmp-house-2012-v1.1.dta")
house_2012_is <- wmp_house_2012_v1_1 %>%
  filter(race == 'US HOUSE') %>%
  sjlabelled::as_label(party, ad_tone, per_ply) %>%
  select(creative, market, dmaid, issue, airdate, airtime, categorystate,
         race, district, party, ad_tone, per_ply, est_cost, station) %>%
  mutate(cycle = substr(airdate,0,4)) %>%
  rename(state = categorystate, dma = dmaid) %>%
  mutate(state = abbr2state(state) ) %>%
  mutate(st_fips = case_when(state == 'Alabama' ~ '01',
                             state == 'Alaska' ~ '02',
                             state == 'Arizona' ~ '04',
                             state == 'Arkansas' ~ '05',
                             state =='California' ~ '06',
                             state =='Colorado' ~ '08',
                             state =='Connecticut' ~ '09',
                             state =='Delaware' ~ '10',
                             state == 'Florida' ~ '12',
                             state == 'Georgia' ~ '13',
                             state == 'Hawaii' ~ '15',
                             state =='Idaho' ~ '16',
                             state =='Illinois' ~ '17',
                             state =='Indiana' ~ '18',
                             state =='Iowa' ~ '19',
                             state =='Kansas' ~ '20',
                             state =='Kentucky' ~ '21',
                             state =='Louisiana' ~ '22',
                             state =='Maine' ~ '23',
                             state =='Maryland' ~ '24',
                             state =='Massachusetts' ~ '25',
                             state =='Michigan' ~ '26',
                             state =='Minnesota' ~ '27',
                             state =='Mississippi' ~ '28',
                             state =='Missouri' ~ '29',
                             state =='Montana' ~ '30',
                             state =='Nebraska' ~ '31',
                             state =='Nevada' ~ '32',
                             state =='New Hampshire' ~ '33',
                             state =='New Jersey' ~ '34',
                             state =='New Mexico' ~ '35',
                             state =='New York' ~ '36',
                             state =='North Carolina' ~ '37',
                             state =='North Dakota' ~ '38',
                             state =='Ohio' ~ '39',
                             state =='Oklahoma' ~ '40',
                             state =='Oregon' ~ '41',
                             state =='Pennsylvania' ~ '42',
                             state =='Rhode Island' ~ '44',
                             state =='South Carolina' ~ '45',
                             state =='South Dakota' ~ '46',
                             state =='Tennessee' ~ '47',
                             state =='Texas' ~ '48',
                             state =='Utah' ~ '49',
                             state =='Vermont' ~ '50',
                             state =='Virginia' ~ '51',
                             state =='Washington' ~ '53',
                             state =='West Virginia' ~ '54',
                             state =='Wisconsin' ~ '55',
                             state =='Wyoming' ~ '56')) 
head(house_2012)

# st_cd_fips
nums <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
house_2012_is$cd_fips <- ifelse(house_2012_is$district %in% nums, 
                                paste0("0",house_2012_is$district), 
                                house_2012_is$district)

house_2012_is$st_cd_fips <- paste(house_2012_is$st_fips,
                                  house_2012_is$cd_fips,
                                  sep = "")
head(house_2012_is)
dim(house_2012_is)

# check names
names(house_2012_is)

# 2014
WMP_house_gen_2014_ads_cleaned <- readRDS("~/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/WMP/House ads/WMP_house_gen_2014_ads_cleaned.rds")
house_2014_is <- WMP_house_gen_2014_ads_cleaned %>%
  filter(race == 'US HOUSE') %>%
  sjlabelled::as_label(party, ad_tone, per_ply) %>%
  select(creative, market, dma, issue, airdate, airtime, StateAbb,
         race, district, affiliation, ad_tone, per_ply, est_cost, station) %>% # issue not included bc diff dimensions
  mutate(cycle = substr(airdate,0,4)) %>%
  rename(party = affiliation, state = StateAbb) %>%
  mutate(state = abbr2state(state) ) %>%
  mutate(st_fips = case_when(state == 'Alabama' ~ '01',
                             state == 'Alaska' ~ '02',
                             state == 'Arizona' ~ '04',
                             state == 'Arkansas' ~ '05',
                             state =='California' ~ '06',
                             state =='Colorado' ~ '08',
                             state =='Connecticut' ~ '09',
                             state =='Delaware' ~ '10',
                             state == 'Florida' ~ '12',
                             state == 'Georgia' ~ '13',
                             state == 'Hawaii' ~ '15',
                             state =='Idaho' ~ '16',
                             state =='Illinois' ~ '17',
                             state =='Indiana' ~ '18',
                             state =='Iowa' ~ '19',
                             state =='Kansas' ~ '20',
                             state =='Kentucky' ~ '21',
                             state =='Louisiana' ~ '22',
                             state =='Maine' ~ '23',
                             state =='Maryland' ~ '24',
                             state =='Massachusetts' ~ '25',
                             state =='Michigan' ~ '26',
                             state =='Minnesota' ~ '27',
                             state =='Mississippi' ~ '28',
                             state =='Missouri' ~ '29',
                             state =='Montana' ~ '30',
                             state =='Nebraska' ~ '31',
                             state =='Nevada' ~ '32',
                             state =='New Hampshire' ~ '33',
                             state =='New Jersey' ~ '34',
                             state =='New Mexico' ~ '35',
                             state =='New York' ~ '36',
                             state =='North Carolina' ~ '37',
                             state =='North Dakota' ~ '38',
                             state =='Ohio' ~ '39',
                             state =='Oklahoma' ~ '40',
                             state =='Oregon' ~ '41',
                             state =='Pennsylvania' ~ '42',
                             state =='Rhode Island' ~ '44',
                             state =='South Carolina' ~ '45',
                             state =='South Dakota' ~ '46',
                             state =='Tennessee' ~ '47',
                             state =='Texas' ~ '48',
                             state =='Utah' ~ '49',
                             state =='Vermont' ~ '50',
                             state =='Virginia' ~ '51',
                             state =='Washington' ~ '53',
                             state =='West Virginia' ~ '54',
                             state =='Wisconsin' ~ '55',
                             state =='Wyoming' ~ '56')) 
head(house_2014_is)

# st_cd_fips
nums <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
house_2014_is$cd_fips <- ifelse(house_2014_is$district %in% nums, 
                                paste0("0",house_2014_is$district), 
                                house_2014_is$district)

house_2014_is$st_cd_fips <- paste(house_2014_is$st_fips,
                                  house_2014_is$cd_fips,
                                  sep = "")
head(house_2014_is)

house_2014_is <- house_2014_is %>%
  mutate(per_ply = case_when(per_ply == 0 ~ "Neither",
                             per_ply == 1 ~ "Personal Characteristics",
                             per_ply == 2 ~ "Policy Matters",
                             per_ply == 3 ~ "Both Personal Characteristics and Policy Matters"),
         ad_tone = case_when(ad_tone == 1 ~ "Contrast",
                             ad_tone == 2 ~ "Promote", 
                             ad_tone == 3 ~ "Attack"))
head(house_2014_is)

# 2016
WMP_house_gen_2016_ads_cleaned <- readRDS("~/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/WMP/House ads/WMP_house_gen_2016_ads_cleaned.rds")
house_2016_is <- WMP_house_gen_2016_ads_cleaned %>%
  filter(race == 'US HOUSE') %>%
  sjlabelled::as_label(party, ad_tone, per_ply) %>%
  select(creative, market, dma, issue, airdate, airtime, categorystate,
         race, district, Candidate_Party, ad_tone, per_ply, est_cost, station) %>% # issue not included bc diff dimensions
  mutate(cycle = substr(airdate,0,4)) %>%
  rename(party = Candidate_Party, state = categorystate) %>%
  mutate(state = abbr2state(state) ) %>%
  mutate(st_fips = case_when(state == 'Alabama' ~ '01',
                             state == 'Alaska' ~ '02',
                             state == 'Arizona' ~ '04',
                             state == 'Arkansas' ~ '05',
                             state =='California' ~ '06',
                             state =='Colorado' ~ '08',
                             state =='Connecticut' ~ '09',
                             state =='Delaware' ~ '10',
                             state == 'Florida' ~ '12',
                             state == 'Georgia' ~ '13',
                             state == 'Hawaii' ~ '15',
                             state =='Idaho' ~ '16',
                             state =='Illinois' ~ '17',
                             state =='Indiana' ~ '18',
                             state =='Iowa' ~ '19',
                             state =='Kansas' ~ '20',
                             state =='Kentucky' ~ '21',
                             state =='Louisiana' ~ '22',
                             state =='Maine' ~ '23',
                             state =='Maryland' ~ '24',
                             state =='Massachusetts' ~ '25',
                             state =='Michigan' ~ '26',
                             state =='Minnesota' ~ '27',
                             state =='Mississippi' ~ '28',
                             state =='Missouri' ~ '29',
                             state =='Montana' ~ '30',
                             state =='Nebraska' ~ '31',
                             state =='Nevada' ~ '32',
                             state =='New Hampshire' ~ '33',
                             state =='New Jersey' ~ '34',
                             state =='New Mexico' ~ '35',
                             state =='New York' ~ '36',
                             state =='North Carolina' ~ '37',
                             state =='North Dakota' ~ '38',
                             state =='Ohio' ~ '39',
                             state =='Oklahoma' ~ '40',
                             state =='Oregon' ~ '41',
                             state =='Pennsylvania' ~ '42',
                             state =='Rhode Island' ~ '44',
                             state =='South Carolina' ~ '45',
                             state =='South Dakota' ~ '46',
                             state =='Tennessee' ~ '47',
                             state =='Texas' ~ '48',
                             state =='Utah' ~ '49',
                             state =='Vermont' ~ '50',
                             state =='Virginia' ~ '51',
                             state =='Washington' ~ '53',
                             state =='West Virginia' ~ '54',
                             state =='Wisconsin' ~ '55',
                             state =='Wyoming' ~ '56')) 
head(house_2016_is)

# st_cd_fips
nums <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
house_2016_is$cd_fips <- ifelse(house_2016_is$district %in% nums, 
                                paste0("0",house_2016_is$district), 
                                house_2016_is$district)

house_2016_is$st_cd_fips <- paste(house_2016_is$st_fips,
                                  house_2016_is$cd_fips,
                                  sep = "")
head(house_2016_is)

# 2018
WMP_house_gen_2018_ads_cleaned <- readRDS("~/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/WMP/House ads/WMP_house_gen_2018_ads_cleaned.rds")
house_2018_is <- WMP_house_gen_2018_ads_cleaned %>%
  filter(race == 'US HOUSE') %>%
  sjlabelled::as_label(party, ad_tone, per_ply) %>%
  select(creative, market, dma, issue, airdate, airtime, categorystate,
         race, district, Candidate_Party, ad_tone, per_ply, est_cost, station) %>% # issue not included bc diff dimensions
  mutate(cycle = substr(airdate,0,4)) %>%
  rename(party = Candidate_Party, state = categorystate) %>%
  mutate(state = abbr2state(state) ) %>%
  mutate(st_fips = case_when(state == 'Alabama' ~ '01',
                             state == 'Alaska' ~ '02',
                             state == 'Arizona' ~ '04',
                             state == 'Arkansas' ~ '05',
                             state =='California' ~ '06',
                             state =='Colorado' ~ '08',
                             state =='Connecticut' ~ '09',
                             state =='Delaware' ~ '10',
                             state == 'Florida' ~ '12',
                             state == 'Georgia' ~ '13',
                             state == 'Hawaii' ~ '15',
                             state =='Idaho' ~ '16',
                             state =='Illinois' ~ '17',
                             state =='Indiana' ~ '18',
                             state =='Iowa' ~ '19',
                             state =='Kansas' ~ '20',
                             state =='Kentucky' ~ '21',
                             state =='Louisiana' ~ '22',
                             state =='Maine' ~ '23',
                             state =='Maryland' ~ '24',
                             state =='Massachusetts' ~ '25',
                             state =='Michigan' ~ '26',
                             state =='Minnesota' ~ '27',
                             state =='Mississippi' ~ '28',
                             state =='Missouri' ~ '29',
                             state =='Montana' ~ '30',
                             state =='Nebraska' ~ '31',
                             state =='Nevada' ~ '32',
                             state =='New Hampshire' ~ '33',
                             state =='New Jersey' ~ '34',
                             state =='New Mexico' ~ '35',
                             state =='New York' ~ '36',
                             state =='North Carolina' ~ '37',
                             state =='North Dakota' ~ '38',
                             state =='Ohio' ~ '39',
                             state =='Oklahoma' ~ '40',
                             state =='Oregon' ~ '41',
                             state =='Pennsylvania' ~ '42',
                             state =='Rhode Island' ~ '44',
                             state =='South Carolina' ~ '45',
                             state =='South Dakota' ~ '46',
                             state =='Tennessee' ~ '47',
                             state =='Texas' ~ '48',
                             state =='Utah' ~ '49',
                             state =='Vermont' ~ '50',
                             state =='Virginia' ~ '51',
                             state =='Washington' ~ '53',
                             state =='West Virginia' ~ '54',
                             state =='Wisconsin' ~ '55',
                             state =='Wyoming' ~ '56')) 
head(house_2018_is)

# st_cd_fips
nums <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
house_2018_is$cd_fips <- ifelse(house_2018_is$district %in% nums, 
                                paste0("0",house_2018_is$district), 
                                house_2018_is$district)

house_2018_is$st_cd_fips <- paste(house_2018_is$st_fips,
                                  house_2018_is$cd_fips,
                                  sep = "")
head(house_2018_is)

# join now
names(house_2012_is)
names(house_2014_is)
names(house_2016_is)
names(house_2018_is)

h_is <- house_2012_is %>%
  rbind(house_2014_is) %>%
  rbind(house_2016_is) %>%
  rbind(house_2018_is)

dim(h_is)

# check name matching again for ad_tone and party
table(h$party)
h_is <- h_is %>%
  mutate(party = case_when(party == 'Democrat' ~ 'Democrat',
                           party == 'DEMOCRAT' ~ 'Democrat',
                           party == 'Democratic' ~ 'Democrat',
                           party == 'REPUBLICAN' ~ 'Republican',
                           party == 'Republican' ~ 'Republican')) %>%
  mutate(ad_tone = tolower(ad_tone)) %>%
  filter(cycle %in% c(2006, 2010, 2012, 2014, 2016, 2018)) %>%
  rename(ad_purpose = per_ply) %>%
  mutate(ad_purpose = tolower(ad_purpose), ad_purpose = case_when(ad_purpose == 'both personal char. and policy matters' ~ 
                                                                    "both",
                                                                  ad_purpose == 'both personal characteristics and policy matters' ~ 
                                                                    "both",
                                                                  ad_purpose == 'personal characteristics' ~
                                                                    "personal",
                                                                  ad_purpose == 'policy matters' ~
                                                                    "policy"))

table(h_is$party)
table(h_is$ad_tone)
table(h_is$ad_purpose)
table(h_is$cycle)
table(unique(h_is$issue))
table(h_is$est_cost)

dim(h_is)

# csv
write.csv(h_is, "/Users/kiara/Dropbox/ElectionAnalytics_Midterms/Lab sessions/campaigns I_ air war/ads_issues_2012-2018.csv")